...
const ProjectActions = () => {
  ...
  const addMilestone = () => {
    setFormData(prev => ({
      ...prev,
      milestones: [...prev.milestones, { name: '', description: '', fundNeeded: '' }]
    }));
  };
  ...
  const handleSubmitProject = async (e) => {
    ...
    try {
      ...
      if (Math.abs(totalMilestonesFunding - parseFloat(formData.goal)) > 0.0001) {
        addNotification(
      ...
      const formattedMilestones = formData.milestones.map(m => ({
        ...
        fundNeeded: ethers.utils.parseEther(m.fundNeeded),
        isComplete: false,
        voting: false,
        isApproved: false,
        totalVoters: 0,
      ...
      console.log(`Creating project "${formData.name}" with ${formData.milestones.length} milestones`);
      
      // Send the notification to prepare user for wallet popup
      addNotification('Creating project, please confirm in your wallet...', 'info', 0);
      ...
      try {
        ...
        addNotification(`Transaction submitted! Waiting for confirmation on blockchain...`, 'info', 0);
        
        // Wait for transaction confirmation
        await tx.wait(1); // Wait for 1 confirmation
        
        // Show success notification
        addNotification(`Project "${formData.name}" successfully created!`, 'success');
        ...
        if (txError.code === 4001 || (txError.message && txError.message.includes("user rejected"))) {
          errorMessage = "Transaction was rejected in your wallet";
        }
        // Handle gas estimation errors
        else if (txError.code === "UNPREDICTABLE_GAS_LIMIT") {
          errorMessage = "Gas estimation failed. Contract rejected the transaction parameters.";
        } 
        // Generic error
  ...
  if (!isWagmiConnected && !window.location.href.includes('dashboard')) {
    return <div className="p-4">Please connect your wallet to interact with projects</div>;
  }
      ...
      <div className="bg-black p-6 rounded-lg shadow-md border border-gray-800">
        ...
        <form onSubmit={handleSubmitProject}>
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-white">Fundraising Goal (ETH)</label>
              <input
                type="number"
                name="goal"
                value={formData.goal}
            ...
            <div>
              ...
              {formData.milestones.map((milestone, index) => (
                <div key={index} className="p-4 border border-gray-600 bg-gray-800 rounded-md mb-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-white">Funds Needed (ETH)</label>
                      <input
                        type="number"
                        value={milestone.fundNeeded}
                        onChange={(e) => handleMilestoneChange(index, 'fundNeeded', e.target.value)}
                        className="mt-1 block w-full rounded-md border-gray-600 bg-gray-800 text-white shadow-sm focus:border-[#FF7171] focus:ring-[#FF7171]"
                        step="0.01"
                        required
                      />
